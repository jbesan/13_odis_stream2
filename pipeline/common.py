...
def load_dataset(path: Path, config: Dict[str, Any], **kwargs) -> pd.DataFrame:
    ...
    if fmt == "parquet":
        ...
        excel_kwargs = {
            ...
            if k not in ["sheet_name", "header", "skiprows"]
        }
...
def fetch_remote_metadata_datagouv(resource_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logging.info(
            f"📡 Resolving data.gouv.fr stable redirect for resource: {resource_id}"
        )
        # Use stream=True to only load response headers without downloading the body
        response = requests.get(
            url, headers=headers, stream=True, allow_redirects=True, timeout=15
        )
        response.raise_for_status()

        last_modified_header = response.headers.get("Last-Modified")
        logging.info(
            f"Metadata header for {resource_id}: Last-Modified = {last_modified_header}"
        )
        ...
        if last_modified_header:
            ...
            return {"last_modified": dt.isoformat(), "url": response.url}
        else:
            logging.warning(
                f"⚠️ No 'Last-Modified' header found in response for resource {resource_id}"
            )
            return None
...
def validate_dataset_contract(
    df: pd.DataFrame, source_name: str, source_cfg: Dict[str, Any]
) -> bool:
    ...
    if df is None or len(df) == 0:
        logging.warning(
            f"⚠️ [CONTRACT VALIDATION FAILED] Dataset '{source_name}' is empty or None."
        )
        return False
    ...
    if used_cols:
        ...
        if missing_cols:
            logging.warning(
                f"⚠️ [CONTRACT VALIDATION WARNING] Dataset '{source_name}' is missing columns: {missing_cols}"
            )
            if len(missing_cols) == len(used_cols):
                logging.error(
                    f"❌ [CONTRACT VALIDATION FAILED] Dataset '{source_name}' is missing ALL expected columns."
                )
                return False
    ...
    for pk in pk_cols:
        ...
        if null_count > 0:
            ...
            if pct_null > 5.0:
                logging.warning(
                    f"⚠️ [CONTRACT VALIDATION FAILED] Dataset '{source_name}' has high null rate ({pct_null:.1f}%) in primary identifier '{pk}'."
                )
                return False

    logging.info(
        f"✅ [CONTRACT PASSED] Dataset '{source_name}' matches schema contract."
    )
    return True
...
def finalize_ingest(
    ...
    config: Dict[str, Any],
    is_staging: bool,
    staging_raw: Optional[Path],
    staging_clean: Path,
    active_raw: Optional[Path],
    active_clean: Path,
) -> bool:
    """
    ...
    try:
        if not staging_clean.exists():
            logging.warning(
                f"⚠️ Staging clean file not found for {source_name}: {staging_clean}"
            )
            return False
