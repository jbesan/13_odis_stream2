# Données géographiques

## Communes + Dep + Reg + EPCI + Polygons (1km)

- Données: https://www.data.gouv.fr/datasets/contours-administratifs/#/resources/12f8cdc9-5844-4666-a4b5-6385d5415f67
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/12f8cdc9-5844-4666-a4b5-6385d5415f67
- Typ de fichier: geojson

## Population

- Données: https://www.data.gouv.fr/datasets/population-municipale-des-communes-france-entiere/#/resources/630e7917-02db-4838-8856-09235719551c
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/630e7917-02db-4838-8856-09235719551c
- Type de fichier: xlsx
- Onglet utiles: III_1_insee_population_fr_commu

## Communes Voisines

- Données: https://www.data.gouv.fr/datasets/liste-des-adjacences-des-communes-francaises/#/resources/6ca4e113-271d-45bb-8995-5d9779f7b10d
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/6ca4e113-271d-45bb-8995-5d9779f7b10d
- Typ de fichier: csv

## COG

- Données: https://www.data.gouv.fr/datasets/code-officiel-geographique-cog/#/resources/91a95bee-c7c8-45f9-a8aa-f14cc4697545
- URL Stable: https://object.files.data.gouv.fr/hydra-parquet/hydra-parquet/b65c0022cfa4e00c71bee1af980652bf.parquet
- Type de fichier: parquet

## Bassins de vie

- Page: https://www.insee.fr/fr/information/6676988
- Lien direct: https://www.insee.fr/fr/statistiques/fichier/6676988/BV2022_au_01-01-2025.zip
- Type de fichier: zip

## Codes Postaux

- Page: https://www.data.gouv.fr/datasets/base-officielle-des-codes-postaux/
- Lien direct: https://object.files.data.gouv.fr/hydra-parquet/hydra-parquet/008a2dda-2c60-4b63-b910-998f6f818089.parquet
- Type de fichier: parquet

# Inclusion

## Services Inclusion

- Données: https://www.data.gouv.fr/api/1/datasets/r/de4856b8-bb67-4b79-83bd-038e846a1774
- Référentiel: csv/referentiel_services_inclusion.csv
- Type de fichier: csv

## Couleur politique

- Données: https://www.data.gouv.fr/datasets/communes-enrichies-avec-la-nuance-politique-france/#/resources/ea5d6bc3-37d0-4884-a437-155a90c3e05f
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/ea5d6bc3-37d0-4884-a437-155a90c3e05f
- Type de fichier: csv

## Associations

- Données: https://www.data.gouv.fr/datasets/rna-agrege-a-lechelle-nationale/#/resources/cc7b8f0c-45ea-4444-8b55-55d30bc34ac5
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/cc7b8f0c-45ea-4444-8b55-55d30bc34ac5
- Type de fichier: parquet

# Logement

## Logement vacants parc privé

- Données: https://www.data.gouv.fr/datasets/logements-vacants-du-parc-prive-en-france-et-par-commune-departement-region/
- Lien direct: https://www.data.gouv.fr/api/1/datasets/r/2e0417b4-902d-4c60-90e7-bf5df148cb87
- Type de fichier: csv

## Logements Sociaux

- Données: https://www.statistiques.developpement-durable.gouv.fr/54-millions-de-logements-locatifs-sociaux-en-france-au-1er-janvier-2024#:~:text=Au%201er%20janvier%202024%2C%20la,11%20400%20ont%20%C3%A9t%C3%A9%20vendus.
- Lien direct: https://www.statistiques.developpement-durable.gouv.fr/media/7970/download?inline
- Type de fichier: xlsx

## Taux d'occupation par commune

- Données: https://www.data.gouv.fr/datasets/logements/#/resources/cae4e8a4-15e0-40ff-88fe-087b9e206514
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/cae4e8a4-15e0-40ff-88fe-087b9e206514
- Type de fichier: zip
- Fichiers utiles: DS_RP_LOGEMENT_COMPL_2022_data.csv

# Education

## Petite enfance

- Données: https://www.data.gouv.fr/datasets/taux-de-couverture-daccueil-du-jeune-enfant-communal/
- Lien direct: https://www.data.gouv.fr/api/1/datasets/r/6d3b1cf8-cbef-4f30-96c3-0b63a2b0502c
- Type de fichier: json

## Effectifs Ecoles

- Données: https://www.data.gouv.fr/datasets/effectifs-deleves-par-niveau-et-nombre-de-classes-par-ecole-date-dobservation-au-debut-du-mois-doctobre-chaque-annee/#/resources/b4642552-6777-4265-a263-f190621ec40f
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/b4642552-6777-4265-a263-f190621ec40f
- Type de fichier: parquet

## Annuaire Education

- Données: https://www.data.gouv.fr/datasets/annuaire-de-leducation/#/resources/9eb02ac9-4bce-4fa8-a6f7-451c5b366f66
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/9eb02ac9-4bce-4fa8-a6f7-451c5b366f66
- Type de fichier: geojson

## Niveau d'occupation des logements

# Emploi

## Besoin en main d'oeuvre (BMO)

- Données: https://www.data.gouv.fr/datasets/enquete-besoins-en-main-doeuvre-bmo/#/resources/54478315-aafd-4070-b8df-a5647297e0c0
- Lien direct: https://www.data.gouv.fr/api/1/datasets/r/54478315-aafd-4070-b8df-a5647297e0c0
- Type de fichier: xlsx

## Référentiel Communes -> Bassins d'Emploi (2021)

- Page: https://statistiques.francetravail.org/bmo/static/methode_2021
- Lien direct: https://statistiques.pole-emploi.org/bmo/Handlers/HTFile.ashx?MEDIAID=156688&SITEKEY=620c8132-d522-44fa-8efa-27a62c26702c
- Type de fichier: xlsx
- Onglet utiles: Feuil1

## Référentiel métiers FAP 2021

- Page: https://www.data.gouv.fr/datasets/la-nomenclature-des-familles-professionnelles-2021/#/resources/f1be98eb-0414-4648-a683-7dbed3ebaca7
- Lien direct: https://www.data.gouv.fr/api/1/datasets/r/f1be98eb-0414-4648-a683-7dbed3ebaca7
- Type de fichier: csv
- colonnes utiles: Code FAP 228, Intitulé FAP 228

## Population Active & Chomage

- Données: https://www.data.gouv.fr/datasets/population-active-et-chomage/
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/f43b668d-251e-472a-9385-3d006d1ca6b8
- Type de fichier: zip
- Fichiers utiles: DS_RP_EMPLOI_LR_COMP_2022_data.csv

## Salaires secteurs privé

- Données: https://www.data.gouv.fr/datasets/salaires-dans-le-secteur-prive-selon-le-sexe-et-la-categorie-socioprofessionnelle-au-niveau-communal/

## Référentiel emploi ROME

- Données: https://www.data.gouv.fr/datasets/repertoire-operationnel-des-metiers-et-des-emplois-rome/
  NE MARCHE PAS

## Formations

- Données: https://www.data.gouv.fr/fr/datasets/liste-publique-des-organismes-de-formation-l-6351-7-1-du-code-du-travail/
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/803121dd-1acf-423d-b7d8-8511f1c607c1
- Type de fichier: xlsx

# Santé

## Annuaire santé FINESS

- Données: https://www.data.gouv.fr/datasets/finess-extraction-du-fichier-des-etablissements/#/resources/98f3161f-79ff-4f16-8f6a-6d571a80fea2
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/98f3161f-79ff-4f16-8f6a-6d571a80fea2
- Type de fichier: csv

## Annuaire Maternités

- Données: https://www.data.gouv.fr/datasets/liste-des-maternites-de-france-depuis-2000/#/resources/a822f7ae-7c46-42c0-86e1-ad7d381c1ad2
- URL Stable: https://www.data.gouv.fr/api/1/datasets/r/a822f7ae-7c46-42c0-86e1-ad7d381c1ad2
- Type de fichier: json
