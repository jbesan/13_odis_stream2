...
**Directives** :
1. **Recherche Google Search** : Utilise ton accès natif à Google Search pour répondre aux questions sur :
    - L'actualité récente de la ville ({FOCUS_CITY}).
...
2. **Réponse** :
    ...
    - Ton retour sera fusionné avec celui de l'expert terrain (Maps).
"""

class WebAgent(BaseAgent):
    def run(self, message: str, context: AgentContext) -> str:
        focus_city = context.focus_city or "la ville demandée"
        prompt = WEB_PROMPT.replace("{FOCUS_CITY}", focus_city)
