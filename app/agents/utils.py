...
def launch_background_city_analysis(
    ...
    def bg_analysis_task(results_store: dict):
        ...
        try:
            logging.info(
                f"🚀 [BG-ANALYSIS] Starting background analysis for {nom} ({codgeo})"
            )
            ...
            execution_mode = (
                "specific_ask" if messages and len(messages) > 1 else "full_analysis"
            )
            ...
            try:
                ...
                logging.info(
                    f"✅ [BG-ANALYSIS] Background thread finished for {nom} ({codgeo})"
                )
            except Exception as e:
                logging.error(
                    f"❌ [BG-ANALYSIS] Agent Error for {nom} ({codgeo}): {e}",
                    exc_info=True,
                )
                current_val = results_store.get(task_key, {})
            ...
            logging.error(
                f"❌ [BG-ANALYSIS] Setup Error for {nom} ({codgeo}): {global_e}"
            )
            current_val = results_store.get(task_key, {})
            current_val["status"] = "error"
