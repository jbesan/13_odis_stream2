...
from app.agents.tools import compute_top_cities
from app.models import SearchCriterias

def test_compute_top_cities_wrapper():
    # Use context managers for patching to avoid pytest fixture conflicts
    with patch("app.agents.tools._compute_top_cities_logic") as mock_logic, \
         patch("app.agents.tools.WEIGHT_PROFILES", {"Équilibré": {"mock": "weights"}, "Famille": {"mock": "famille_weights"}}):
