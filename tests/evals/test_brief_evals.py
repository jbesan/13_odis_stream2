...
async def test_brief_scenarios_evaluation():
    ...
    for scenario in scenarios:
        logfire.info(
            "Running evaluation test for scenario: {name}",
        ...
        dataset = Dataset(
            name="odis_brief_eval_dataset", cases=[case], evaluators=[judge]
        )
        ...
        assert assertion_result.value is True, (
            f"Refinement evaluation failed: {assertion_result.reason}"
        )
