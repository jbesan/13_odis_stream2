...
import pytest
import numpy as np
from app.scoring import compute_category_scores
...
def mock_config():
    return ScoringConfig(
        ...
        commune_actuelle='33063', loc_distance_km=50,
        nb_adultes=1, nb_enfants=0,
        hebergement='Location', logement='Location',
        codes_metiers=[], codes_formations=[], classe_enfants=[],
        besoin_sante='Aucun', besoins_autres={},
        socle_admin_selection=[], affinite_selection=[],
        binome_penalty=0.0, pop_min=0,
        criteria_weights={}
    )
...
def test_weighted_average(mock_config):
    ...
    # Row 0: (0*1 + 1*1) / 2 = 0.5
    # Row 1: (1*1 + 0*1) / 2 = 0.5
    df_res = compute_category_scores(df, scores_cat, 0.0, mock_config)
