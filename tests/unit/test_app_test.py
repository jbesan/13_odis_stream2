...
@patch("ui.ui_telemetry.log_page_view")
@patch("utils.auth.check_password", return_value=True)
def test_main_app_redirect_authenticated(mock_auth, mock_page_view, mock_initialize):
...
@patch("ui.ui_telemetry.log_page_view")
@patch("utils.auth.check_password", return_value=True)
def test_accueil_page_runs_authenticated(mock_auth, mock_page_view, mock_initialize):
    accueil_py = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../app/pages/1_Accueil.py")
    )
...
def test_analytics_page_blocks_non_admin(mock_is_admin, mock_auth, mock_page_view):
    analytics_py = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../app/pages/4_Analytics.py")
    )
