...
def make_search_criterias(raw_scenario_data: dict) -> SearchCriterias:
    ...
    inc_services_selection = [{"code": s, "label": s} for s in raw_data.get('inc_services_selection', [])]
    inc_asso_add_selection = [{"code": a, "label": a} for a in raw_data.get('inc_asso_add_selection', [])]
    ...
    criterias_dict = {
        ...
        'notes_qualitatives': [raw_data['notes_qualitatives']] if isinstance(raw_data.get('notes_qualitatives'), str) else raw_data.get('notes_qualitatives', []),
        ...
        'org_strategic_locations_type': raw_data.get('org_strategic_locations_type', 'departement'),
...
def test_tier1_demo_scenarios_integration(real_engine, app_data, scenario_id):
    ...
    for cat in real_engine.categories:
        ...
        if weight > 0:
            assert col_name in scored_df.columns, f"Category '{cat}' is weighted ({weight}) but '{col_name}' column is missing from results!"
    ...
    baseline_ids = real_engine.scores_cat[real_engine.scores_cat['baseline'] == True]['score'].tolist()
    for b_id in baseline_ids:
        # Check if the category of the baseline criteria is excluded
        cat_of_baseline = real_engine.scores_cat[real_engine.scores_cat['score'] == b_id].iloc[0]['cat']
        if cat_of_baseline == 'education' and config.nb_enfants == 0:
            continue
        if cat_of_baseline == 'sante' and config.besoin_sante == 'Aucun':
            continue
        assert b_id in config.active_criteria, f"Baseline criteria '{b_id}' is not in active_criteria set!"
    ...
    for cat in real_engine.categories:
        ...
        if col_name in scored_df.columns:
            ...
            assert ((scores >= 0.0) & (scores <= 1.0)).all(), f"Scores in '{col_name}' are out of bounds [0.0, 1.0]!"
...
def test_tier2_bounded_output_invariant(real_engine):
    ...
    assert ((scored_df['weighted_score'] >= 0.0) & (scored_df['weighted_score'] <= 1.0)).all()
...
def test_tier2_data_model_parity(real_engine):
    ...
    for i, result_item in enumerate(model.results[:10]):
        ...
        assert pytest.approx(result_item.global_score, abs=1e-4) == df_row['weighted_score']
        
        # Parity for territory category score if present in model
        if hasattr(result_item, 'territoire') and result_item.territoire and 'territoire_cat_score' in df_row.index:
...
def test_tier2_config_engine_category_symmetry(real_engine):
    ...
    for cat in yaml_baseline_cats:
        col_name = f"{cat}_cat_score"
        assert col_name in scored_df.columns, f"Category '{cat}' is in scores_config.yaml as baseline but '{col_name}' is missing in scored DataFrame!"
...
def test_tier2_monotonic_weight_influence(real_engine):
    ...
    config_low = make_search_criterias({'poids_emploi': 0.1, 'poids_logement': 0.9, 'weight_profile': 'Profil personnalisé'})
    config_high = make_search_criterias({'poids_emploi': 1.0, 'poids_logement': 0.9, 'weight_profile': 'Profil personnalisé'})
    ...
    if len(high_emp_communes) > 0:
        ...
        assert df_high.loc[commune_id, 'weighted_score'] >= df_low.loc[commune_id, 'weighted_score'] - 1e-6
...
def test_tier2_fixture_parity_invariant(real_engine, live_scores_cat):
    ...
    yaml_cats = {item.get('category') for item in scores_yaml.get('scores', []) if item.get('category')}
    
    fixture_cats = set(live_scores_cat['cat'].unique())
    
    missing_cats = yaml_cats - fixture_cats
    assert not missing_cats, f"conftest.py live_scores_cat fixture is missing categories: {missing_cats}"
