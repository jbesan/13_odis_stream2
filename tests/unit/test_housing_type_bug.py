...
def mock_data_context():
    ...
    mock_odis.__getitem__.side_effect = lambda x: (
        MagicMock() if isinstance(x, str) else mock_odis
    )
...
def test_type_logement_propagation(mock_data_context):
    ...
    mock_engine.run_optimized.return_value = (
        MagicMock(results=[]),
        MagicMock(),
    )  # Return (SearchResultsData, GDF)
    ...
    try:
        ...
        assert config.type_logement.code == "appt_t1_t2", (
            f"Expected appt_t1_t2, got {config.type_logement}"
        )
        ...
        assert config_default.type_logement.code == "appt_all", (
            f"Expected appt_all by default, got {config_default.type_logement}"
        )
