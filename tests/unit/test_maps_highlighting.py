...
from core.maps import build_scores_layer
...
def test_build_scores_layer_highlights_commune(monkeypatch):
    ...
    mock_app_data = {"odis_geo": pd.Series({commune_code: commune_poly})}
    ...
    # Assert
    layers = [
        child for child in fg._children.values() if isinstance(child, folium.GeoJson)
    ]
    ...
    assert max_coord == 1.0, (
        "Expected Commune polygon (max coord 1.0), but got something else."
    )
