...
from agents.social_integration_expert import social_integration_expert_agent, SocialIntegrationResult
...
async def test_ts_agent_planning_offline(mock_deps):
    ...
    with ts_agent.override(model=FunctionModel(call_model)):
        result = await ts_agent.run(
            "Fais une analyse complète pour Saint-Jean-d'Angély.",
            deps=mock_deps
        )
...
async def test_housing_agent_tool_calling_offline(mock_deps):
    ...
    with housing_expert_agent.override(model=FunctionModel(call_model)):
        with patch('agents.housing_expert.search_places_batch', new_callable=AsyncMock) as mock_search, \
             patch('agents.housing_expert.search_ccas', return_value=[{'name': 'CCAS local'}]) as mock_ccas:
            mock_search.return_value = [{'name': 'CADA 1'}]
            
            result = await housing_expert_agent.run(
                "Quels sont les hébergements et le CCAS ?",
                deps=mock_deps
            )
            
            mock_search.assert_called_once_with(['CADA', 'CHRS'], "Saint-Jean-d'Angély, Nouvelle-Aquitaine")
...
async def test_mobility_agent_tool_calling_offline(mock_deps):
    ...
    with mobility_expert_agent.override(model=FunctionModel(call_model)):
        with patch('agents.mobility_expert.compute_routes', return_value={'duration': '1h30'}) as mock_routes:
            result = await mobility_expert_agent.run(
                "Combien de temps pour aller à Bordeaux en train ?",
                deps=mock_deps
            )
            
            mock_routes.assert_called_once_with("Saint-Jean-d'Angély", "Bordeaux", "transit")
...
async def test_job_hunter_agent_tool_calling_offline(mock_deps):
    ...
    def call_model(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
            args = {
                'searches': [{'location': '17347', 'rome': 'I1604'}]
            }
            return ModelResponse(parts=[ToolCallPart('search_job_offers_batch_tool', args)])
        else:
            result_tool_name = info.output_tools[0].name if info.output_tools else 'final_result'
            args = {
    ...
    with job_hunter_agent.override(model=FunctionModel(call_model)):
        with patch('agents.job_hunter.search_job_offers_batch', new_callable=AsyncMock) as mock_jobs:
            mock_jobs.return_value = {'offres': [{'id': '123'}]}
            
            result = await job_hunter_agent.run(
                "Cherche des offres de mécanicien.",
                deps=mock_deps
            )
            
            mock_jobs.assert_called_once_with([{'location': '17347', 'rome': 'I1604'}])
            assert isinstance(result.output, JobHunterResult)
            assert result.output.searched == "Offres mécanicien (I1604) à Saint-Jean-d'Angély"
...
async def test_social_integration_agent_tool_calling_offline(mock_deps):
    ...
    def call_model(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
            args = {
                'queries': ['cours de français', 'football'],
                'codgeo': '17347'
            }
            return ModelResponse(parts=[ToolCallPart('search_rna_rag_batch_tool', args)])
        else:
            result_tool_name = info.output_tools[0].name if info.output_tools else 'final_result'
            args = {
    ...
    with social_integration_expert_agent.override(model=FunctionModel(call_model)):
        with patch('agents.social_integration_expert.search_rna_rag_batch', new_callable=AsyncMock) as mock_rna:
            mock_rna.return_value = [{'name': 'Club Foot'}]
            
            result = await social_integration_expert_agent.run(
                "Cherche des cours de français et du football.",
                deps=mock_deps
            )
            
            mock_rna.assert_called_once_with(['cours de français', 'football'], '17347', top_k=10)
            assert isinstance(result.output, SocialIntegrationResult)
            assert result.output.searched == "Recherche associations de français et football"
