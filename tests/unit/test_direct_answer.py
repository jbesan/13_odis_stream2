...
def mock_deps():
    ...
    state.search_criteria = SearchCriterias(
        commune_actuelle=CriteriaItem(code="13001", label="Marseille"), nb_adultes=1
    )
    current_hash = compute_criteria_hash(state.search_criteria)
    state.criteria_hash = current_hash

    marseille = CommuneResult(
        codgeo="13001", name="Marseille", population=800000, global_score=0.9
    )
    state.search_results = SearchResultsData(
        search_hash=current_hash, results=[marseille], current_geo=marseille
    )
    state.focus_city = CommuneResult(name="Marseille", codgeo="13001")
    state.messages.append(
        {"role": "user", "content": "Quel est le loyer moyen à Marseille ?"}
    )
...
async def test_direct_answer_bypass(mock_deps):
    ...
    ts_plan = SwarmPlan(
        ...
        direct_answer="Le loyer moyen à Marseille est de 15€/m².",
        tasks=[],
    )
