...
def test_search_referentiels_inclusion(mcp_data_context):
    ...
    results = _search_referentiels_logic(
        "Apprendre Français", domain="inclusion_services"
    )
    assert len(results) > 0
    top = results[0]
    assert (
        "français" in top["label"].lower()
        or "fle" in top["label"].lower()
        or "langue" in top["label"].lower()
    )
