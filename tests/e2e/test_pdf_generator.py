...
def sample_session_state():
    ...
    mock_app_data = {
        ...
        "annuaire_inclusion": pd.DataFrame(
            {"codgeo": ["33063"], "categorie": ["social"], "service": ["aide-sociale"]}
        ),
    }
...
def sample_search_results():
    ...
    results = [
        CommuneResult(
            ...
            inclusion=InclusionMetrics(
                asso_refugee_list=[
                    AssociationDetail(
                        id="1",
                        name="Asso Refugee Paris",
                        waldec_label="Social",
                        description="Helping people",
                    )
                ],
                asso_inclusion_list_by_cat={
                    "Culture": [
                        AssociationDetail(
                            id="2",
                            name="Asso Culture Paris",
        ...
        CommuneResult(
            codgeo="69123", name="Lyon", population=500000, global_score=0.72
        ),
    ]

    current_geo = CommuneResult(
        codgeo="33063", name="Bordeaux", population=250000, global_score=0.65
    )

    return SearchResultsData(
        search_hash="test_hash", results=results, current_geo=current_geo
    )
