...
def test_formation_codes_intersection():
    ...
    if not formations_df.empty and 'formation_code' in formations_df.columns:
         formations_df['formation_code'] = formations_df['formation_code'].astype(str).str.replace(r'\.0$', '', regex=True)
    ...
    assert len(intersection) > 0, "No common codes found between formations data and referentiel!"
    
    # Optional: Check strict format (no ".0" suffix)
    # This ensures we really fixed the clean format, not just made the test pass
    has_decimal_suffix = any(str(c).endswith('.0') for c in data_codes if str(c).replace('.0','').isdigit())
